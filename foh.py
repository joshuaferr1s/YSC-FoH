...
def report(movie, movie_totals, movie_timedata, early_screening = False):
    ...
    if not early_sceening:
        ...
        print(dividor)
        try:
            multiplier = movie_totals['Total'] / pre_bets['Total']
...
def load_movie_data(movie):
	...
	movie_timedata = dict{}
	try:
        movie_totals = load_movie_data('movies/' + movie + '.json')
    ...
    try:
    	movie_timedata = load_movie_data('movies/' + movie + '_timedata.json')
    except FileNotFoundError:
    	pass
...
while running:
    ...
    if choice == 'quit':
    	...
    	if movie in movies_recorded:
    		...
    		movie_totals, movie_timedata = record(True, movie, early_sceening)
    	else:
            movie_totals, movie_timedata = record(False, movie, early_sceening)
        ...
        if report == 'yes':
        	report(movie, movie_totals, movie_timedata)
        elif report == 'no':
        	pass
        else:
